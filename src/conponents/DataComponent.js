...
const CombinedComponent = () => {
  ...
  const fadeInRight = {
    ...
    visible: { opacity: 1, x: 0, transition: { duration: .8, ease: "easeInOut" } },
  };
    ...
    <div className="px-4 sm:px-6 lg:px-8">
      ...
      >
        <h1 className="text-2xl sm:text-4xl mt-6 lg:text-5xl font-extrabold text-gray-900 mb-4 tracking-wide flex justify-center items-center gap-2">
          <img src={img.leaveimage} alt="Decorative Icon" className="w-6 h-6 sm:w-8 sm:h-8 inline-block" />
          WHO WE ARE
        </h1>
        <p className="text-xs sm:text-md lg:text-lg text-gray-600 font-medium mb-6 sm:mb-11">
        ...
        <div className="mt-10 sm:mt-16 max-w-7xl mx-auto">
          ...
          >
            {[
              ...
              <motion.div
                key={index}
                className={`p-3 sm:p-5 rounded-md shadow-md flex flex-col justify-center items-center ${
                  index % 2 === 0 ? "bg-white text-gray-800 border border-gray-800" : "bg-yellow-900 text-white"
                }`}
                variants={fadeInUp} // Apply scrolling animation
              >
                <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold">{stat.value}</h2>
      ...
      <motion.div
        ...
        className="text-center mt-10 sm:mt-16"
      >
        <p className="text-xs sm:text-md lg:text-lg text-gray-700 leading-relaxed mt-6 mb-9 px-2 sm:px-6 max-w-4xl mx-auto">
        ...
        <motion.p
          className="text-xs sm:text-md lg:text-lg text-gray-700 leading-relaxed mt-10 px-2 sm:px-6 max-w-4xl mx-auto"
          variants={fadeInUp}
          initial="hidden"
          whileInView="visible"
