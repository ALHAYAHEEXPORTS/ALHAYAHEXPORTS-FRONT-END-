...
const WhyChooseUs = () => {
  ...
  return (
    <motion.div
      className="py-8 px-4 mt-10 sm:px-6 lg:px-12 bg-[#fbfdf7e8]"
      initial="hidden"
      whileInView="visible"
      viewport={{ once: true, amount: 0.2 }}
      variants={fadeInUp}
    >
      <h2 className="text-xl sm:text-3xl lg:text-4xl font-extrabold text-center mb-6 text-[#4B4E6D] flex items-center justify-center gap-2 sm:gap-3">
        <img
          src={img.leaveimage}
      ...
      <p className="text-center text-gray-600 max-w-3xl mx-auto mb-8 sm:mb-10 text-xs sm:text-base lg:text-lg">
      ...
      {/* Card Row */}
      <motion.div
        className="flex flex-wrap justify-center gap-3 sm:gap-6 lg:gap-8 mb-8"
        variants={fadeInUp}
      >
        {Object.keys(sections).map((key) => (
          <motion.div
            ...
            className={`cursor-pointer p-3 sm:p-4 rounded-md w-24 sm:w-40 lg:w-56 aspect-square transition-all duration-200 ${
      ...
      <motion.div
        className="text-center max-w-3xl mx-auto p-3 sm:p-6 lg:p-8 bg-white rounded-md shadow-md"
        variants={fadeInUp}
      >
