...
const RiceCalculator = () => {
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      try {
        const res = await fetch("http://localhost:8000/api/rice-prices");
  ...
  useEffect(() => {
    ...
    blendInputs.forEach((blend) => {
      ...
      cost += pricePerMT * (blend.quantity || 0);
    });
  ...
  return (
    <div className="max-w-4xl mt-28 mx-auto p-6 bg-[#f8f6f6] rounded-lg shadow-lg ">
      <h1 className="text-3xl font-bold mb-6 text-center">Rice Price Calculator</h1>

      <div className="mb-8">
        <h2 className="text-xl font-semibold mb-4">Rice Blends</h2>
        {blendInputs.map((blend, index) => (
          <div key={index} className="flex flex-wrap gap-4 mb-4 items-end">
            <div className="flex-1 min-w-[200px]">
              <label className="block mb-1 font-medium">Rice Type</label>
              <select
                value={blend.type}
            ...
            <div className="w-1/4 min-w-[120px]">
              <label className="block mb-1 font-medium">Blend %</label>
              <input
                type="number"
            ...
            <div className="w-1/4 min-w-[120px]">
              <label className="block mb-1 font-medium">Quantity (MT)</label>
              <input
                type="number"
      ...
      <div className="mb-8">
        <h2 className="text-xl font-semibold mb-4">⚙️ Additional Costs</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label className="block mb-1 font-medium">Freight (Per MT)</label>
            <input
              type="number"
              value={freight}
          ...
          <div>
            <label className="block mb-1 font-medium">Processing Charges (Per MT)</label>
            <input
              type="number"
      ...
      <div className="bg-green-50 p-6 rounded text-center">
        <h2 className="text-xl font-semibold mb-2">💰 Total Estimated Cost</h2>
        <p className="text-3xl font-bold text-green-700">${totalCost} USD</p>
        {lastUpdated && (
